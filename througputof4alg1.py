...
I[2][0] = 1
I[2][3] = 1
q = [[0 for col in range(4)] for row in range(4)]
for m in range(0, 4):
    for n in range(0, 4):
        q[m][n] = Queue.Queue(maxsize=count)

channel = [[0 for col in range(4)] for row in range(4)]
...
for m in range(0, 4):
    k[m] = 1.0 / (delay_constraint[m] * arrival_lambda[m] * channel_lambda[m] * dcf)

K = 600.0
trans_count = [[0 for col in range(loops)] for row in range(4)]
schedule = [[0 for col in range(1)] for row in range(4)]
arrival1 = [[0 for col in range(4)] for row in range(4)]

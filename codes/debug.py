...
for xx in iw_file_list:
    ...
    remain = []
    del_l = 0
    max_last = int(qu_r[0][3])
    try:
        debug_f = open("debug.csv", 'wb')
        d_w = csv.writer(debug_f)
    except Exception:
        print 'open debug.csv failed'
    d_w.writerows(qu_r)
    if debug_f:
        debug_f.close()
    for i in range(1, tmp_len - 1):
        ...
        if curr < max_last:
            ...
            remain.append(qu_r[i])
            max_last = int(qu_r[i][3])
    # qu_r = qu_r[(index_record + 1):]
    qu_r = remain
    level = float(len(remain)) / float(tmp_len)
    if level < 0.9:
        continue
    xtmp = [len(remain), tmp_len, round(
        float(len(remain)) / float(tmp_len), 3), qu_f]
    delete_list.append(xtmp)
    fname = wr_f.replace('stats', 'debug')
    try:
        dbg_f = open(fname, 'wb')
        dbg_w = csv.writer(dbg_f)
    except Exception:
        print 'debug_f open failed'
    dbg_w.writerows(remain)
    if dbg_f:
        dbg_f.close()
    # break
    # condition = 2

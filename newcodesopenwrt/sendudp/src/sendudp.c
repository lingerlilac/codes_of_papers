...
extern struct tcp_flow flow_linger[100];
...
static void recv_cb(struct sk_buff *skb)
{
    ...
    if(skb->len >= nlmsg_total_size(0))
    {
        ...
        if(!strcmp(data, "flows"))
        {
            ...
            char tmp_1[25];
            // int struct_len = sizeof(struct tcp_flow);
        ...
        else if(!strcmp(data, "states"))
        {
    		...
    		duration = (float)((tmp2.sec - tmp1.sec) * 1000 + (tmp2.usec - tmp1.usec) / 1000);
    		apstat.drop_count = ((float)(tmp2.drop_count - tmp1.drop_count)) / duration;
    		dur_sur = (__u32)(tmp2.time - tmp1.time);
    		apstat.time_busy = ((float)(tmp2.time_busy - tmp1.time_busy)) / dur_sur;
    		apstat.time_ext_busy = ((float)(tmp2.time_ext_busy - tmp1.time_ext_busy)) / dur_sur;
    		apstat.time_rx = ((float)(tmp2.time_rx - tmp1.time_rx)) / dur_sur;
    		apstat.time_tx = ((float)(tmp2.time_tx - tmp1.time_tx)) / dur_sur;
    		apstat.time_scan = ((float)(tmp2.time_scan - tmp1.time_scan)) /dur_sur;
    		apstat.noise = tmp2.noise;
    		apstat.packets = ((float)(tmp2.packets - tmp1.packets)) / duration;
    		apstat.bytes = ((float)(tmp2.bytes - tmp1.bytes)) / duration;
    		apstat.qlen = tmp2.qlen;
    		apstat.backlog = tmp2.backlog;
    		apstat.drops = ((float)(tmp2.drops - tmp1.drops)) / duration;
    		apstat.requeues = ((float)(tmp2.requeues - tmp1.requeues)) / duration;
    		apstat.overlimits = ((float)(tmp2.overlimits - tmp1.overlimits)) / duration;
...
void get_information(void)
{
	...
	if (queue)
	{
		struct apstates *tmp;
		tmp = apstates_global + aps_head;
		tmp->packets = queue->packets;
		tmp->bytes = queue->bytes;
		tmp->qlen = queue->qlen;
		tmp->backlog = queue->backlog;
		tmp->drops = queue->drops;
		tmp->requeues = queue->requeues;
		tmp->overlimits = queue->overlimits;
		tmp->sec = queue->sec;
		tmp->usec = queue->usec;		
